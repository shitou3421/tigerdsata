...
class MFaker:
    ...
    @staticmethod
    def get(var, *args, **kwargs):
        """获取faker的方法"""
        print(var)
        if (args is None) or (kwargs is None):
            return getattr(MFaker().f, var)
        return getattr(MFaker().f, var)(*args, **kwargs)

    @staticmethod
    def get_nums(func, nums, *args, **kwargs):
        data_list = []
        for i in range(nums):
            data_list.append(MFaker().get(func, *args, **kwargs))
        return data_list


if __name__ == '__main__':
    # for i in range(10):
        # print(MFaker.get("pyint", min_value=8888, max_value=9999))
    print(MFaker.get("pystr", min_chars=1, max_chars=255).__repr__())
    print(MFaker.get("pystr", None).__repr__())
